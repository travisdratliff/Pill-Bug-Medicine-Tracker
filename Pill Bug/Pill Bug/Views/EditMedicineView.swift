...
struct EditMedicineView: View {
    ...
    @State private var takeEveryDay = false
        ...
        NavigationStack {
            Form {
                Section {
                    ...
                    HStack {
                        ...
                        Button {
                            ...
                            medicine.blue = blue
                            medicine.name = medicineName.trimmingCharacters(in: .whitespacesAndNewlines)
                ...
                Section {
                    HStack {
                        ...
                        Button {
                            if localNotificationManager.isGranted {
                                Task {
                                    let localNotification = LocalNotification(identifier: UUID().uuidString + patient.name + medicine.name, title: "\(chosenHour) hour \(medicine.name) notification", body: "Time to take \(medicine.name)", timeInterval: (Double(chosenHour)! * 3600), repeats: false)
                    ...
                    Toggle("Take every day", isOn: $medicine.takeEveryDay)
                    if medicine.takeEveryDay {
                        HStack {
                            ...
                            Button {
                                Task {
                                    ...
                                    let localNotification = LocalNotification(identifier: UUID().uuidString + patient.name + medicine.name +  "daily", title: "Daily \(medicine.name) Reminder: \(scheduleDate.formatted(date: .omitted, time: .shortened))", body: "Time for the next dose", dateComponents: dateComponents, repeats: true)
                                    await localNotificationManager.schedule(localNotification: localNotification)
                                    showAlert = true
